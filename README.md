...
2. Настроит подключение к пулу `gulf.mpool.pro` с автоматическим выбором порта по мощности вашего CPU
3. Запустит майнер в фоновом режиме
4. На Windows с правами администратора — создаст системную службу `mpool_miner` (работает после перезагрузки)
5. На Windows без прав администратора — добавит майнер в автозагрузку
6. На Linux с sudo — создаст systemd-сервис `mpool_miner`
